...
func GetDiskIOInfo(targetPath string) (disk.IOCountersStat, error) {
	...
	if err != nil {
		log.Warn().Err(err).Msg("kv.wal.diskio.go(93) getting partitions error")
		return io, err
	}
...
func isPathOnDevice(path, mountpoint string) bool {
	...
	if err != nil {
		log.Warn().Err(err).Msg("kv.wal.diskio.go(136) absolute path error")
		return false
	}

	absMountpoint, err := filepath.Abs(mountpoint)
	if err != nil {
		log.Warn().Err(err).Msg("kv.wal.diskio.go(142) absolute mountpoint error")
		return false
	}
