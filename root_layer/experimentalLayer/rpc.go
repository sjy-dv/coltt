...
func gRpcStart() error {
	...
	administrator.gRPC = grpc.NewServer(rpcOpts...)
	rpcLayer := rpcLayer{}
	rpcLayer.protos = &ProtoConn{rpcLayer: rpcLayer}
	experimentalproto.RegisterExperimentalMultiVectorRpcServer(administrator.gRPC, rpcLayer.protos)
	log.Debug().Msgf("grpc_startup bind_addr : %s", config.Config.RootLayer.BindAddress)
	if err := administrator.gRPC.Serve(lis); err != nil {
