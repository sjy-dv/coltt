...
func gRpcStart() error {
	...
	rc.S = grpc.NewServer(rpcOpts...)
	rpcLayer := rpcLayer{}
	rpcLayer.Cproto = &coreProtoConn{rpcLayer: rpcLayer}
	//rpcLayer.rootClone = xx
	coreproto.RegisterCoreRpcServer(rc.S, rpcLayer.Cproto)
	log.Debug().Msgf("grpc_startup bind_addr : %s", config.Config.RootLayer.BindAddress)
	if err := rc.S.Serve(lis); err != nil {
