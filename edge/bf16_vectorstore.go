...
func (qx *bf16vecSpace) InsertVector(collectionName string, commitId uint64, data ENode) error {
	...
	qx.vertices[shardIdx][commitId] = ENodeBF16{Vector: lower, Metadata: data.Metadata}
	return nil
}

func (qx *bf16vecSpace) UpdateVector(collectionName string, id uint64, data ENode) error {
	if qx.distance.Type() == T_COSINE {
		data.Vector = Normalize(data.Vector)
	}
	lower, err := qx.quantization.Lower(data.Vector)
	...
	qx.verticesMu[shardIdx].Lock()
	defer qx.verticesMu[shardIdx].Unlock()
	qx.vertices[shardIdx][id] = ENodeBF16{Vector: lower, Metadata: data.Metadata}
	return nil
}
...
func (qx *bf16vecSpace) FullScan(collectionName string, target Vector, topK int, highCpu bool,
